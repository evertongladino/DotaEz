...
<html lang="en">
  ...
  <body>
	...
	<div class="container">
		<?php
		if(isset($_SESSION['nme_user'])){
			echo '<a class="btn btn-warning" href="form-new-item.php">Novo Item</a><br>';
			echo '<a class="btn btn-warning" href="form-alter-item.php">Alterar Item</a>';
		}
		?>
		<h2 class="sub-header">Todos os itens</h2>
			...
			$get_request = 'http://localhost/dotaserver/item/';
			$response = \Httpful\Request::get($get_request)->send();
			$response->body;
			$arr = json_decode($response->body, true);
			foreach ($arr as $key =>$value){
				echo '<tr><td>'.$value['idt'].'</td>'.'<td>'.$value['nme_item'].'</td>'.'<td>'.$value['desc_item'].'</td></tr>';
			}
			?>
			</tbody>
			</table>
